...
// 多字段选取（标准写法）
user.Select(true, user.Id, user.Name);

// 单字段选取
user.Select(user.GetName());

// WHERE 中引用
qb.Where($"{user.GetStatus()} = {user.AsParam("Status")}").AddParameter("Status", 1);
```
